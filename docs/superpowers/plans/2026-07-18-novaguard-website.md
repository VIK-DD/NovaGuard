...
- All copy in English. Creator credit preserved verbatim: `Designed & Developed by VIK & CloudMediaSRL`.
...
Footer: hairline top border; left — `NovaGuard` serif wordmark + creator credit exactly `Designed & Developed by VIK & CloudMediaSRL`; right — links `/commands`, `/dashboard`, `/coming-soon` (label "Classic page"), `/privacy`, `/terms`, GitHub repo.
